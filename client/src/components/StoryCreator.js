...
const choiceList = (choice, getNodeTitle) => (<span className="node-editor-link">{choice.choiceText}: {getNodeTitle(choice.nextNode)}</span>);

const nodeCard = (node, changeNodeHandler, getNodeTitle) => 
    (
        <div className="tile">
            <button onClick={() => changeNodeHandler(node._id)}>Edit</button>
            <p>{node.nodeTitle}</p>
            <p>Links: {node.choices && node.choices.length ? node.choices.map(choice => choiceList(choice, getNodeTitle)) : "End"}</p>
        </div>
    );

const nodeDisplay = (storyNodes, changeNodeHandler, getNodeTitle) =>
    (
        <div className="tile-display">
            {Object.keys(storyNodes)
                .map(id => nodeCard(storyNodes[id], changeNodeHandler, getNodeTitle))}
        </div>
    );
...
export default class StoryCreator extends Component {
    ...
    //getting the node name for the nodeCard choices display
    getNodeTitle = (nodeId) => this.state.storyNodes[nodeId].nodeTitle;
    ...
    render() {
        return (
            <div>
                ...
                {nodeDisplay(this.state.storyNodes, this.changeCurrentNode, this.getNodeTitle)}
            </div>
        );
    }
}